...
/// A simple view that renders a single definition line segment with embedded tappable links
    ...
    var body: some View {
        FlowLayout(spacing: 4) {
            ForEach(Array(definition.segments.enumerated()), id: \.offset) { _, segment in
                switch segment {
                case .text(let text):
                    // Split by spaces to preserve word-level wrapping
                ...
                case .link(let word, let displayText):
                    Text(displayText)
                        .font(.system(size: 14, weight: .regular, design: .serif))
...
#Preview {
    VStack(spacing: 20) {
        ...
        DefinitionSegmentView(
            line: "A simple definition without links.",
            onLinkTap: { word in print("Tapped: \(word)") }
        )
        
        // Example with links
        DefinitionSegmentView(
            line: "A [[complicated]] definition with [[link|display text]] to other words.",
            onLinkTap: { word in print("Tapped: \(word)") }
        )
    }
    .background(.black)
